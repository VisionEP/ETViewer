            ...
            _tcsncmp(sTemp, _T("/PDB:"), _tcslen(_T("/PDB:"))) == 0)
        {
            ...
            if(hFind!=INVALID_HANDLE_VALUE)
            {
                do
                {
                    ...
                    _tcscpy_s(tempFile, MAX_PATH, drive);
                    _tcscat_s(tempFile, MAX_PATH, path);
                    _tcscat_s(tempFile, MAX_PATH, findData.cFileName);
#ifdef _DEBUG
                    ASSERT(_tcsstr(tempFile, drive) &&
                           _tcsstr(tempFile, path) &&
                           _tcsstr(tempFile, findData.cFileName));
#endif
